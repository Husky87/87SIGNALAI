...
export const ResearchAssistantView: React.FC<ResearchAssistantViewProps> = ({
  ...
  const getModelLabel = (model: string) => {
    switch (model) {
      ...
      case 'gemini-3.5-flash': return 'Signal87 Standard';
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className="relative flex flex-col bg-[#131314] text-[#e3e3e3] font-sans h-full flex-1 min-h-0 overflow-hidden select-none"
    >
      <input
        type="file"
        ref={fileInputRef}
      ...
      {isDragging && (
        <div className="absolute inset-0 bg-[#131314]/90 backdrop-blur-md z-50 flex flex-col items-center justify-center text-white border-4 border-dashed border-[#7dd3fc] p-6 text-center animate-fadeIn">
          <UploadCloud size={64} className="text-[#7dd3fc] animate-bounce mb-4" />
          <h2 className="text-2xl font-extrabold tracking-tight">Drop files to attach to AI Workspace</h2>
          <p className="text-sm text-[#c4c7c5] max-w-md mt-2">
      ...
      <header className="hidden md:flex h-14 px-4 sm:px-6 items-center justify-between gap-3 flex-shrink-0 z-10 bg-[#131314]">
        <div className="flex items-center gap-3">
          {/* Model Selector Dropdown */}
          <div className="relative">
            <button
              onClick={() => setShowModelMenu(!showModelMenu)}
              className="px-3.5 py-1.5 text-sm rounded-full font-semibold text-[#e3e3e3] hover:bg-[#1e1f20] transition-all flex items-center gap-1.5 cursor-pointer border border-[#37393b]"
            >
              <Sparkles size={15} className="text-[#7dd3fc]" />
              <span>{getModelLabel(selectedModel)}</span>
              <ChevronDown size={14} className="text-[#c4c7c5] ml-0.5" />
            </button>
            {showModelMenu && (
              <div className="absolute left-0 mt-2 w-64 bg-[#1e1f20] border border-[#37393b] rounded-2xl shadow-xl py-1.5 z-50 animate-in fade-in duration-150">
                {[
        ...
        <div className="flex items-center gap-2">
          {currentUser ? (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-[#1e1f20] hover:bg-[#28292a] border border-[#37393b] rounded-full text-xs text-[#e3e3e3] font-medium transition-colors cursor-pointer">
              {currentUser.photoURL ? (
                <img src={currentUser.photoURL} alt={currentUser.displayName || 'User'} className="w-5 h-5 rounded-full object-cover" />
              ) : (
                <div className="w-5 h-5 rounded-full bg-[#004a77] text-[#e3e3e3] font-bold flex items-center justify-center text-[10px]">
          ...
          {chatHistory.length > 0 && (
            <button
              ...
              className="p-1.5 text-[#c4c7c5] hover:text-rose-400 hover:bg-[#1e1f20] rounded-full transition-colors cursor-pointer"
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden relative bg-[#131314]">
        <div className={`flex-1 flex flex-col justify-between min-w-0 h-full overflow-hidden transition-all duration-300 ${
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 sm:py-6 flex flex-col">
            <div className={`max-w-[768px] w-full mx-auto space-y-6 ${chatHistory.length === 0 ? 'flex-1 flex flex-col justify-center my-auto py-2 sm:py-6' : ''}`}>
              {chatHistory.length === 0 ? (
          ...
          <div
            className="flex-shrink-0 bg-[#131314] z-20 px-3 sm:px-4 pt-2 pb-4 sm:pb-3 border-t border-[#28292a]/50"
            style={{
