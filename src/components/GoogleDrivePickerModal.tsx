  ...
  const loadFiles = async (authToken: string) => {
    ...
    try {
      ...
      if (err.message === 'UNAUTHORIZED' || err.message?.includes('401')) {
        setToken(null);
        setError('Google authentication expired. Please click "Connect Google Drive" below to reconnect.');
      } else {
        setError(err.message || 'Failed to access Google Drive files');
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-white">Google Drive Porting Hub</h2>
              ...
              <p className="text-xs text-[#c4c7c5]">
                Select files from your Google Drive for instant OCR, legal parsing & AI memory indexing
              </p>
            </div>
          </div>
          ...
          {!token ? (
            <div className="py-8 px-6 bg-gradient-to-br from-[#1e1f20] to-[#131314] border border-[#37393b] rounded-2xl flex flex-col items-center justify-center text-center space-y-4 my-auto">
              ...
              <div className="max-w-md space-y-2">
                <h3 className="text-lg font-bold text-white">Connect Your Google Drive Account</h3>
              ...
              <div className="flex-1 overflow-y-auto min-h-[220px] max-h-[380px] space-y-1.5 pr-1">
                {loading ? (
                  <div className="py-16 flex flex-col items-center justify-center space-y-3 text-[#c4c7c5]">
                    ...
                    <span className="text-xs font-semibold">Fetching Google Drive directory...</span>
                  </div>
                ) : files.length === 0 ? (
                  <div className="py-16 flex flex-col items-center justify-center space-y-2 text-[#8e918f]">
                    <FolderOpen size={32} />
                    <span className="text-xs font-semibold">No matching Google Drive files found</span>
                    <p className="text-[11px] text-[#c4c7c5]">
                      Try adjusting your search filter or uploading documents to Google Drive.
                    </p>
                  </div>
                ) : (
                  files.map((file) => {
